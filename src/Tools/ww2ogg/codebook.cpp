...
uint32_t read_32_le(const unsigned char b[4])
{
    uint32_t v = 0;
    for (int i = 3; i >= 0; i--)
    {
...
{
    HRSRC hRes = FindResourceA(NULL, MAKEINTRESOURCE(IDR_BIN1), "BIN");
    HGLOBAL hData = LoadResource(NULL, hRes);
    DWORD file_size = SizeofResource(NULL, hRes);
    const char* data = static_cast<const char*>(LockResource(hData));

    long offset_offset = read_32_le(reinterpret_cast<const unsigned char*>(data + file_size - 4));
    codebook_count = (file_size - offset_offset) / 4;
    ...
    codebook_offsets = new long[codebook_count];
    const char* offset_ptr = data + offset_offset;
    for (long i = 0; i < codebook_count; ++i)
    {
        codebook_offsets[i] = read_32_le(reinterpret_cast<const unsigned char*>(offset_ptr + i * 4));
    }
}
